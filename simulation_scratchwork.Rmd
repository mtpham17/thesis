...
analyze <- function(data, p, true_beta) {
  ...
  errors_lambda_range <- sum(unlist(lapply(errors_lambda_range[-1], function(x) ifelse(length(unname(x)) > 0, 1, 0))))
  lambda_range <- lambda_range$result
  
  best_lambda <- quiet_best_lambda(data = data,
                             formula = as.formula(paste("Y ~", covariates)), 
                             lambda_min = lambda_range[1], lambda_max = 
                               lambda_range[2])
  errors_best_lambda <- sum(unlist(lapply(errors_best_lambda[-1], function(x) ifelse(length(unname(x)) > 0, 1, 0))))
  ...
  model_lasso_mixed <- quiet_mixed_lasso(fix = as.formula(paste("Y ~", covariates)),
                                         ...
                                         lambda=best_lambda,
                                         final.re = TRUE)
  errors_model_lasso_mixed <- sum(unlist(lapply(errors_model_lasso_mixed[-1], function(x) ifelse(length(unname(x)) > 0, 1, 0))))
