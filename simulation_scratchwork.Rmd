...
analyze <- function(data, p, fixed_beta) {
  ...
  # deemed a beta as non-zero or both deemed a beta as zero
  
  agreement <- (sum((coefs_df$fixed != 0 & coefs_df$mixed != 0)) + 
                  sum((coefs_df$fixed == 0 & coefs_df$mixed == 0))) / nrow(coefs_df)
  ...
  return(c(agreement, fixed_bet_rmse, mixed_beta_rmse, fixed_accuracy, mixed_accuracy,
...
one_run <- function(j, n = 100, 
  ...
  return(c(type, p, s, default, cov_x, mean_x, sigma_x,
                    cov_rand_fixed,
                    mean_r, sigma_r,
