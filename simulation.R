...
lambda_range <- function(data, p, baseline_min_lambda = 10^(-2), baseline_max_lambda = 10^3,
  ...
  for (l in lambdas) {
    ...
    if (sum(mixed_lasso$coefficients[2:(p + 1)] != 0) == p) {
      min_lambda <- l
      break
    } 
  }
  
  for (l in rev(lambdas)) {
    ...
    if (sum(mixed_lasso$coefficients[2:(p + 1)] != 0) == 0) {
      max_lambda <- l
      break
    }
  }
...
rerun_single_params <- function(chunkNo, seed, reps,
                                ...
                                mean_r, sigma_r, mean_noise, sigma_noise) {
  ...
  row <- c(chunkNo, seed, reps, n_bar, J, alpha,
           ...
           mean_r, sigma_r, mean_noise, sigma_noise, mean_results, sd_results)
                           ...
                           mean_r, sigma_r, mean_noise, sigma_noise) 
{
  ...
  # fname = paste0("/n/home04/thupham17/thesis/results/", chunkNo, "_", seed, ".csv")
  res = NA
  if (!file.exists(fname)) {
    res <- safe_run_sim(chunkNo = chunkNo, seed = seed, reps = reps,
                        ...
                        mean_r = mean_r, sigma_r = sigma_r, mean_noise = mean_noise, 
                        sigma_noise = sigma_noise)
