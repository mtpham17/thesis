...
test_df <- generate_data(sample_size = c(3, 4, 3),
...
analyze <- function(data, p, fixed_beta) {
  ...
  # call cv.glmnet()
  model_lasso_fixed <- cv.glmnet(x = X_fixed_train, y = Y_train, nfolds = 5, alpha = 1)
  ...
  lambda_min <- cv_glmmLasso(data = data_train,
  ...
  # be cautious of this one (think of edge cases)
  # TODO: think about calculating precision, recall, F1 score -- use false positive, false
  # negative language
  ...
  fixed_rmse <- rmse(y_hat_fixed, Y, nrow(data))
  mixed_rmse <- rmse(y_hat_mixed, Y, nrow(data))
