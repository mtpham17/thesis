...
test_df <- generate_data(sample_size = c(30, 40, 30),
...
rerun_single_params <- function(df) {
  ...
  results <- rerun(unname(df[1]), one_run(sample_size = unname(df[2]), 
                                          type = unname(df[3]), p = unname(df[4]), 
                                          s = unname(df[5]), default = unname(df[6]),
                                          cov_x = unname(df[7]), scale = unname(df[8]), 
                                          base_mean_x = unname(df[9]), sigma_x = unname(df[10]), 
                                          cov_rand_fixed = unname(df[11]),
...
```{r}
rerun_mult_parameters <- function(runs_vec, sample_size_vec,
  ...
  combos <- expand.grid(runs = runs_vec, sample_size = sample_size_vec,
