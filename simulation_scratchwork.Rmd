...
# change m to p (conventions)

generate_data <- function(p, j, beta_strength, min_beta, sigma_r, n=100) {
  colnames_df <- c(paste("X", 1:m, sep = ""), "random_beta") 
  random_beta <- rnorm(n=j, mean=0, sigma_r)
  
  # make some of these zero
  fixed_beta <- c(rep(min_beta, (1 - beta_strength) * p), rep(1, beta_strength*p))
  # print(fixed_beta)
  df <- matrix(nrow=n)
  # generate X_j
  for (i in 1:p) {
  ...
  # add a second epsilon for the fixed_betas --> this reflects the reliability of 
  # beta measurements (relaxing the assumptions of how strong our measures are)
  
  data$Y <- rowSums(sweep(data[, 1:p], 2, fixed_beta, "*")) + random_beta + epsilon
