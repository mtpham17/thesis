...
lambdas <- seq(0.01, 500, length = 20)
predictive_coefs_1 <- list()
num_coefs_1 <- list()
...
for (i in 1:length(lambdas)) {
  mixed_lasso <- glmmLasso(data = df_analysis,
                           ...
                           rnd = list(provid=~1),
                           lambda = lambdas[i])
  predictive_coefs_1[[i]] <- 
    mixed_lasso$coefficients[2:(length(cfg.abbrev) + 1)]
  num_coefs_1[[i]] <-
    sum(mixed_lasso$coefficients[2:(length(cfg.abbrev) + 1)] != 0)
