...
data_processing <- function(data) {
  ...
  zero_sd <- apply(select_if(df_analysis_noncor, is.numeric), 2, function(x) sd(x) == 0)
  zero_sd_cols <- colnames(select_if(df_analysis_noncor, is.numeric))[zero_sd]
  print(zero_sd_cols)
  df_analysis_noncor <- df_analysis_noncor %>%
    dplyr::select(-all_of(zero_sd_cols))
  
  df_analysis_noncor <- as.data.frame(df_analysis_noncor)
  cor_matrix <- cor(as.matrix(select_if(df_analysis_noncor, is.numeric)))
  cor_1 <- which(cor_matrix > 0.80, arr.ind = TRUE)
...
```{r}
all_vars <- colnames(df_analysis_largec)[! (colnames(df_analysis_largec) %in% 
                                              c("provid", outcome))]
...
# some preprocessing, running LASSO with low regularization parameter of 0.1
...
# TODO at ghungroo: investigate correlation matrix of train data frame...
l_range <- lambda_range(df_analysis_largec, formula, length(coefs_for_glm))
best_lambda <- cv_glmmLasso(df_analysis_largec, formula, rand = list(provid = ~1),
                            lambda_step = 20, lambda_min = l_range[1],
                            lambda_max = l_range[2], kk = 2)
...
hist(coefs_df$Estimate[coefs_df$Estimate < 500 & coefs_df$Estimate > -500])
...
- snr = 0.46
```{r}
colnames(df_analysis_largec)
# "o_c_verbal_Fuss.Cry_FC"                                    
#   [5] "o_c_verbal_Listening_L"                                    
#   [6] "o_c_verbal_Talk_T"                                         
#   [7] "o_c_verbal_TalkSounds_TS" 
```
...
for (outcome in outcomes_use_y1) {
  ...
  df_analysis <- outcomes_and_obs_full_y1_largec %>%
    ...
    # make sure to not use cid, because it's basically just the row number at this point
    dplyr::select(-c(other_outcomes, caretype)) %>%
