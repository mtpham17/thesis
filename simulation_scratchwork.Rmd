...
# these parameters are the most "extreme" values
lambda_range <- function(data, p, baseline_min_lambda = 10^(-2), baseline_max_lambda = 10^4,
                         lambda_no = 100) {
    ...
    for (l in lambdas) {
      mixed_lasso <- glmmLasso(fix = as.formula(paste("Y ~", covariates)),
                           ...
                           lambda = l,
                           final.re = TRUE)
      print(mixed_lasso$coefficients[2:(p + 1)])
...
rerun_single_params <- function(chunkNo, seed, reps,
  ...
  # results <- rerun(df[["reps"]], one_run(n_bar = df[["n_bar"]], J = df[["J"]],
  #                                        alpha = df[["alpha"]],
  #                                        type = df[["type"]], p = df[["p"]],
  #                                        s = df[["s"]], default = df[["default"]],
  #                                        cov_x = df[["cov_x"]], scale = df[["scale"]],
  #                                        base_mean_x = df[["base_mean_x"]],
  #                                        sigma_x = df[["sigma_x"]],
  #                                        mean_r = df[["mean_r"]], sigma_r = df[["sigma_r"]],
  #                                        mean_noise = df[["mean_noise"]],
  #                                        sigma_noise = df[["sigma_noise"]]))
...
file_saving_sim = function(chunkNo, seed, reps,
                           ...
                           mean_r, sigma_r, mean_noise, sigma_noise) 
  {
    ...
    # fname = paste0("/n/home04/thupham17/fasrc/results/", chunkNo, "_", seed, ".csv")
    res = NA
