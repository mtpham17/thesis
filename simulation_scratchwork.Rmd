...
generate_data <- function(n_bar, J, alpha, 
  ...
  mean_x <- matrix(rep(rep(base_mean_x, n) + scale * random_beta, p), nrow = n, ncol = p)
  ...
  sigma_noise <- diag(sigma_noise^2, p, p)
  sigma_noise[outer(1:p, 1:p, function(i,j) i!=j)] <- 1
  measurement_noise <- rmvnorm(n = n, mean = rep(mean_noise, p), sigma_noise)
