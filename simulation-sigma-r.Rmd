...
test_results <- analyze(data = test_df, p = 185, 
                        true_beta = test_beta, default = 1,
                        snr = 1.34)
...
```{r}
# Analyze
min_lambdas <- read.csv(paste0(root, "min_lambdas.csv"))
analyze <- function(data, p, true_beta, default, snr) {
  ...
  l_range <- c(min_lambdas[min_lambdas$snr == snr, "lambda_min"],
               min_lambdas[min_lambdas$snr == snr, "lambda_max"])
  print(l_range)
...
one_run <- function(n_bar, J, alpha, 
  ...
  generate <- generate_data(n_bar, J, alpha, 
                    type, p, s, default,
                    cov_x, scale, base_mean_x, sigma_x, 
                    mean_r, sigma_r, 
                    snr,
                    mean_noise, sigma_noise = 1)
  
  data <- generate$data
  
  fixed_beta <- generate$betas
  result <- analyze(data, p, fixed_beta, default, snr)
  return(result)
}
...
file_saving_sim = function(chunkNo, seed, reps,
  ...
  {
    fname = paste0(root, "results/with_data_icc/",
               chunkNo, "_", seed, ".csv")
    res = NA
...
write.csv(dataset[[1]], 
          paste0(root, "concat_results/with_icc.csv"))
```
