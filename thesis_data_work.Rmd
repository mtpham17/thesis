...
```{r}
library(regclass)
# now that we have the three outcomes we want to work with, quickly run linear regression and observe R^2 value
...
for (outcome in outcomes_use_y1) {
  ...
  df_lreg <- outcomes_and_obs_full_y1 %>% select(-c(other_outcomes))
  lreg <- lm(as.formula(paste(outcome, "~ . - cid")), data = df_lreg, na.action = na.exclude)
  print(summary(lreg)$adj.r.squared)
  print(VIF(lreg))
}
...
```{r}
# need to import lambda helper functions for glmmLasso, but maybe after you figure out which covariates are problematic and causing the Fisher matrix problem?
```
...
## importing cross validation LASSO from other doc

```{r}
lambda_step <- 20
lambdas <- seq(lambda_range_1[1], lambda_range_1[2], length = lambda_step)
...
for (j in 1:lambda_step) {
  ...
  data_train <- df_analysis[-indi,]
  data_test <- df_analysis[indi, ]
  glmmLasso(fix = formula, rnd = list(provid=~1),
            data = data_train, lambda = lambdas[j])
  }
}
```
...
# trying to figure out this problematic data set ... 
data_train %>% dplyr::select(starts_with(c("o_c_focus"))) %>%
  dplyr::select(-ends_with(c("classmean", "classsd")))
```
...
lambda_range <- function(data, formula, p,
                         baseline_min_lambda = 10^(-2), baseline_max_lambda = 10^3,
                         lambda_no = 20) {
...
df_analysis <- outcomes_and_obs_full_y1_largec %>%
  filter(!is.na(outcomes_and_obs_full_y1_largec[[name]])) %>%
  dplyr::select(-c(other_outcomes, caretype, classid, cid, nsweeps.y,
                   actualtype, nsweeps.x)) %>%
  # dropping column that ends with NA so we avoid the "full rank" problem
  dplyr::select(-ends_with(c("NA")))

# df_analysis <- df_analysis[colSums(is.na(df_analysis) | df_analysis == 0) 
#                            != nrow(df_analysis)]

df_analysis <- as.data.frame(df_analysis)

options(na.action="na.pass")
x = model.matrix(as.formula(paste(outcome, "~ . -provid")), data = df_analysis)
...
cfg.abbrev <- coefs_for_glm[c(1:27, 29:length(coefs_for_glm))]
## now run these on glmmLasso:

# 28th variable, "o_c_typetask_TimeOut_TO" seems to be an issue..
# at first, thought this was an issue with the column being all zeros (but it's actually not...) so I'm still not sure what the issue is :(


formula <- as.formula(paste(outcome, "~", paste(cfg.abbrev, collapse= " + ")))
lambda_range_1 <- lambda_range(df_analysis, formula, length(cfg.abbrev))
```


```{r}
mixed_lasso_1 <- cv_glmmLasso(data = df_analysis, formula, 
                              rand=list(provid=~1),
                              lambda_min = lambda_range_1[1], 
                              lambda_max = lambda_range_1[2]) 

coefs_glm_1 <- mixed_lasso_1$coefficients[2:(length(cfg.abbrev) + 1)]
predictive_coefs_glm_1 <- sum(mixed_lasso_1$coefficients[2:(length(cfg.abbrev) + 1)] 
                              != 0)

```


## Trying to work through problematic variables

```{r}
## trying suggestion of using LASSO with very low regularization parameter, trying a lower regularization parameter
df_analysis <- outcomes_and_obs_full_y1_largec %>%
    filter(!is.na(outcomes_and_obs_full_y1_largec[[name]])) %>%
    dplyr::select(-c(other_outcomes, caretype, classid, cid, nsweeps.y,
                    actualtype, nsweeps.x)) 
...
formula <- as.formula(paste(outcome, "~", paste(cfg.abbrev, collapse= " + ")))
mixed_lasso_2 <- glmmLasso(formula, rnd=list(provid=~1), data=df_analysis, 
                        lambda = 0.1, final.re = TRUE) 

coefs_glm_2 <- mixed_lasso_2$coefficients[2:(length(cfg.abbrev) + 1)]
predictive_coefs_glm_2 <- sum(mixed_lasso_2$coefficients[2:(length(cfg.abbrev) + 1)] 
                              != 0)
```
