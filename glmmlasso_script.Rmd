...
generate_data <- function(mean = 0, j = 2, sd = 1, sigma_r = 8, n = 100) {
  ...
  epsilon <- rnorm(n)
  data$Y <- epsilon + data$X1 + data$X2
  return(data)
}
```
...
one_run <- function() {
  ...
  model_mixed <- glmmLasso(fix = Y ~ X1 + X2, rnd = list(j=~1),
                               data = data, lambda = 0.001,
                               final.re = TRUE)
