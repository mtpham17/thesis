...
# snr_final_1 <- load.all.sims(paste0(root, "results/final_snr"))
# write.csv(snr_final_1, paste0(root, "concat_results/snr_final_1.csv"))
snr_final_2 <- load.all.sims(paste0(root, "results/snr_backup"))
write.csv(snr_final_2, paste0(root, "concat_results/snr_final_2.csv"))
snr_final_2_1 <- load.all.sims(paste0(root, "results/snr_backup_2"))
write.csv(snr_final_2_1, paste0(root, "concat_results/snr_final_3.csv"))
```
...
snr_cluster_2 <- read.csv(paste0("/Users/thupham/Desktop/thesis/concat_results", 
                                "/icc_cluster_2.csv"))
snr_final_1 <- read.csv(paste0("/Users/thupham/Desktop/thesis/concat_results", 
                                "/snr_final_1.csv"))
```
...
```{r}
snr_all <- snr_final_1 %>% 
  group_by(snr, sigma_r, scale) %>%
  summarize(across(mean_adj_r_squared:last_col(), ~ mean(.x, na.rm = TRUE)),
        across(reps, sum), .groups = "keep") %>%
  ungroup()
```
...
snr_agreement_plot <- ggplot(snr_all, mapping = aes(x = snr,
                        y = mean_agreement,
                        ymin = min_agreement,
                        ymax = max_agreement)) +
  geom_line() + 
...
## TODO: import other data from other simulations into these and plot again
