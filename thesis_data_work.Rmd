...
cv_glmmLasso <- function(data, formula, rand=list(j=~1),
  ...
  for (j in 1:lambda_step) {
    ...
    # levels(data_test$provid) <- levels(data_train$provid)
  
    glm <- glmmLasso(fix = formula, rnd = rand, data = data, 
              lambda = lambdas[j])
    # return(glm)
    # print(glm$bic)
    # BIC[j,i]<- glm$bic
...
dplyr::select(-c(all_of(other_outcomes), caretype, classid, 
  ...
  drop_na()
df_analysis_largec <- data_processing(raw_full_data,
                                      cor_value = 0.20)
```
...
best_lambda <- cv_glmmLasso(df_analysis_largec, formula,
                            rand = list(provid = ~1),
                            lambda_step = 5, lambda_min = l_range[1],
