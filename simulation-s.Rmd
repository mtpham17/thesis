...
```{r}
lambda_range_2 <- function(scale) {
  min_lambdas <- c(84.21968, 594.7409, 768.4234, 981.5791, 
                   1000.0000, 1000.0000, 1000.0000)
  max_lambdas <- c(1000, 1000, 1000, 1000, 1500, 1500, 1500)
  scale_vec <- seq(0, 6, by = 1)
  return(c(min_lambdas[scale_vec == scale],
           max_lambdas[scale_vec == scale]))
}
```
...
# set.seed(17)
# reps <- 20
# pathname <- paste0(root, "min_lambda_scale/")
# 
# min_lambda_helper <- function(scale) {
#   one_scale_min <- rep(NA, reps)
#   one_scale_max <- rep(NA, reps)
#   for (j in 1:reps) {
...
#     print(l_range)
#     one_scale_min[j] <- l_range[1]
#     one_scale_max[j] <- l_range[2]
#     }
#     df <- data.frame("scale" = scale,
#                 "lambda_min" = mean(one_scale_min),
#                 "lambda_max" = mean(one_scale_max))
#     write.csv(df, paste0(pathname, scale, ".csv"))
#     return(df)
# }
# 
# scale_vec <- c(0, 1, 2, 3, 4, 5, 6)
# scale_df <- data.frame("scale" = scale_vec)
# # write.csv(dataset[[1]],
...
# 
# scale_df$res <- future_pmap(scale_df, .f = min_lambda_helper,
#                       .options = furrr_options(seed = NULL),
#                       .progress = TRUE)
# 
# plan(sequential)
# 
# data <- read.csv(paste0(root, "min_lambda_scale/0.csv"))
# for (scale in scale_vec[-1]) {
#   new <- read.csv(paste0(root, "min_lambda_scale/", scale, ".csv"))
#   data <- rbind(data, new)
# }
# 
# data <- data %>%
#   dplyr::select(-c("X"))
# write.csv(data, paste0(root, "min_lambdas_scale.csv"))
```
...
```{r}
# Analyze
analyze <- function(data, p, true_beta, default, snr, scale) {
  var.names <- paste("X", 1:p, sep = "")
  formula <- as.formula(paste("Y ~", paste(var.names, collapse = " + ")))
...
file_saving_sim = function(chunkNo, seed, reps,
  ...
  {
    fname = paste0(root, "results/icc_scale_2/",
               chunkNo, "_", seed, ".rds")
    res = NA
...
debugging = FALSE
sink(paste0(root, "logs/new_outcome_scale.txt"))
...
pathname <- paste0(root, "results/icc_scale_2")
if (TRUE) {
    # Run in parallel
...
write.csv(dataset[[1]], 
          paste0(root, "concat_results/icc_scale.csv"))
```
