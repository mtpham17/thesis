...
outcomes_vals <- outcomes_and_obs_full_y1_largec[gain_ind] 
pca_results <- prcomp(outcome_vals, scale = TRUE)
pc1 <- pca_results$rotation[, 1]

new_outcome <- rowSums(sweep(outcomes_vals, 2, pc1, "*"))

outcomes_and_obs_full_y1_largec <- cbind(outcomes_and_obs_full_y1_largec,
                                         new_outcome)
outcomes <- c(colnames(outcomes_and_obs_full_y1_largec[, gain_ind]),
              "new_outcome")
```
...
outcome <- outcomes_use[3]
other_outcomes <- setdiff(outcomes, outcome)
raw_full_data <- outcomes_and_obs_full_y1_largec %>%
...
df_analysis_largec <- as.data.frame(data_processing(raw_full_data,
                                      cor_value = 0.80, outcome = outcome))
```
...
cor <- cor(as.matrix(select_if(df_analysis_largec, is.numeric)))
std <- apply(select_if(df_analysis_largec, is.numeric), 2, 
             function(x) sqrt(var(x)))
cov <- cor * std * cor
(sum(cov[1, ], na.rm = TRUE) - 1) / ncol(select_if(df_analysis_largec,
                                                   is.numeric))
# cov_x = -0.0004
```
...
```{r}
all_coefs <- rbind(as.data.frame(mefs_coefs), 
                   as.data.frame(ltr_coefs),
                   as.data.frame(new_outcome_coefs))

all_coefs$outcomes <- factor(c(rep("mefs", nrow(mefs_coefs)),
                               rep("ltr", nrow(ltr_coefs)),
                               rep("new", nrow(new_outcome_coefs))))

write.csv(all_coefs, 
          "/Users/thupham/Desktop/thesis/outcomes_coefs.csv")

all_clusters <- rbind(as.data.frame(mefs_clusters), 
                   as.data.frame(ltr_clusters),
                   as.data.frame(new_outcome_clusters))

all_clusters$outcomes <- factor(c(rep("mefs", nrow(mefs_clusters)),
                               rep("ltr", nrow(ltr_clusters)),
                               rep("new", nrow(new_outcome_clusters))))

write.csv(all_clusters, 
          "/Users/thupham/Desktop/thesis/outcomes_clusters.csv")
```
