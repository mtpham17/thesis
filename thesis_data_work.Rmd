...
df_analysis_singletons <- data_processing(outcomes_and_obs_full_y1)
```


```{r}
pre_lasso <- function(x, y, fixed_lambda, mixed_lambdas = seq(0.01, 500, length = 20)) {
  ...
  mixed_lasso <- glmmLasso(data = df_analysis,
                           fix = formula, 
                           rnd = list(provid=~1),
                           lambda = mixed_lambdas[i])
  predictive_coefs[[i]] <- 
    mixed_lasso$coefficients[2:(length(coefs_for_glm) + 1)]
  num_coefs[[i]] <-
    sum(mixed_lasso$coefficients[2:(length(coefs_for_glm) + 1)] != 0)
...
x_largec = model.matrix(as.formula(paste(outcome, "~ . -provid")), 
                 data = df_analysis_largec)

y_largec = df_analysis_largec[[outcome]]
x_largec = x_largec[, -1]

results_01_largec <- pre_lasso(x_largec, y_largec, 0.1)
results_001_largec <- pre_lasso(x_largec, y_largec, 0.01)

x_singleton = model.matrix(as.formula(paste(outcome, "~ . -provid")), 
                 data = df_analysis_singleton)

y_singleton = df_analysis_singleton[[outcome]]
x_singleton = x_singleton[, -1]
