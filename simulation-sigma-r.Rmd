...
test_generate <- generate_data(n_bar = 3,
                      J = 232,
                      alpha = 1/3,
                      type = 4,
                      p = 185,
...
# test_df <- test_generate$data
# test_beta <- test_generate$betas
# test_results <- analyze(data = test_generate$data, p = test_generate$p,
#                         true_beta = test_generate$betas, default = test_generate$default,
#                         snr = test_generate$snr)
...
```{r}
lambda_range_2 <- function(sigma) {
  # data <- read.csv(paste0(root, "min_lambda_sigma_r.csv"))
  min_lambdas <- c(121.0614, 813.1598, 997.3684, 997.3684, 1000.0000, 1000.0000, 
                   1000.0000, 1000.0000, 1000.0000)
  max_lambdas <- c(1000, 1000, 1000, 1000, 1500, 1500, 1500, 1500, 1500)
  sigma_vec <- seq(0, 4, by = 0.5)
  index <- unlist(lapply(sigma_vec, function(x) all.equal(x, sigma))) == TRUE
...
# set.seed(17)
# reps <- 20
# pathname <- paste0(root, "min_lambda_sigma_r/")
# 
# min_lambda_helper <- function(sigma_r) {
#   one_sigma_min <- rep(NA, reps)
#   one_sigma_max <- rep(NA, reps)
#   for (j in 1:reps) {
...
#                       mean_noise = 0,
#                       sigma_noise = 1)
#     l_range <- lambda_range(test_df$data, test_df$p)
#     print(l_range)
#     one_sigma_min[j] <- l_range[1]
#     one_sigma_max[j] <- l_range[2]
#     }
#     df <- data.frame("sigma_r" = sigma_r,
#                 "lambda_min" = mean(one_sigma_min),
#                 "lambda_max" = mean(one_sigma_max))
#     write.csv(df, paste0(pathname, sigma_r, ".csv"))
#     return(df)
# }
# 
# sigma_vec <- seq(0, 4, by = 0.5)
# sigma_df <- data.frame("sigma_r" = sigma_vec)
# # write.csv(dataset[[1]],
...
# 
# sigma_df$res <- future_pmap(sigma_df, .f = min_lambda_helper,
#                       .options = furrr_options(seed = NULL),
#                       .progress = TRUE)
# 
# plan(sequential)
# 
# data <- read.csv(paste0(root, "min_lambda_sigma_r/0.csv"))
# for (sigma in sigma_vec[-1]) {
#   new <- read.csv(paste0(root, "min_lambda_sigma_r/", sigma, ".csv"))
#   data <- rbind(data, new)
# }
# 
# data <- data %>%
#   arrange(sigma_r)
# write.csv(data, paste0(root, "min_lambda_sigma_r.csv"))
# ```
```
...
```{r}
# Analyze
analyze <- function(data, p, true_beta, default, snr) {
  var.names <- paste("X", 1:p, sep = "")
  formula <- as.formula(paste("Y ~", paste(var.names, collapse = " + ")))
  ...
  return(c(list("adj_r_squared" = summary(basic_linear)$adj.r.squared, 
                "agreement" = agreement,
                "icc" = multilevel.icc(data$Y, cluster = data$j)), 
...
#                       sigma_noise = 1)
# 
# true_betas <- generate_beta(type = 4, p = 185, s = 7)
# results <- analyze(data = test_df, p = 185, true_beta = true_betas,
#                    default = 1, snr = 0.44)
```
...
one_run <- function(n_bar, J, alpha, 
  ...
  generate <- generate_data(n_bar = n_bar, J = J, alpha = alpha, 
  ...
  result <- analyze(data = generate$data, p = generate$p, true_beta = generate$betas, 
                    default = generate$default, snr = generate$snr)
  return(result)
}
...
file_saving_sim = function(chunkNo, seed, reps,
  ...
  {
    fname = paste0(root, "results/icc_sigma_r/",
               chunkNo, "_", seed, ".rds")
    res = NA
...
```{r}
# snr_vec <- seq(0.20, 4.28, 0.24)
# icc <- rep(NA, length(snr_vec))
...
#             sigma_noise = 1)
#   
#   icc[i] <- multilevel.icc(test_df$Y, cluster = test_df$j)
# }
# 
# plot(x = sigma_r_vec, y = icc)
# abline(h = multilevel.icc(df_analysis_largec$gain_c_mefs_str, 
#                           df_analysis_largec$provid))
...
debugging = FALSE
sink(paste0(root, "logs/icc_sigma_r.txt"))
...
pathname <- paste0(root, "results/icc_sigma_r")
if (TRUE) {
    # Run in parallel
...
write.csv(dataset[[1]], 
          paste0(root, "concat_results/icc_sigma_r.csv"))
```
