...
rerun_single_params <- function(df) {
  ...
  results <- rerun(df$reps, one_run(n_bar = df$n_bar, J = df$J, 
                                          ...
                                          base_mean_x = df$base_mean_x, sigma_x = df$sigma_x,
...
```{r}
rerun_mult_parameters <- function(combos) {
...
safe_run_sim = safely(rerun_mult_parameters)
...
if ( TRUE ) {
    ...
    params$results <- future_pmap(params, .f = file_saving_sim,
