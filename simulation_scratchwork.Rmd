...
analyze <- function(data, p, fixed_beta) {
  ...
  agreement <- sum((coefs_df$fixed != 0 & coefs_df$mixed != 0)) / sum(fixed_beta != 0)
  ...
  y_hat_fixed <- predict(model_lasso_fixed, X_fixed)
  y_hat_mixed <- predict(model_lasso_mixed, X_fixed)
  
  fixed_rmse <- rmse(y_hat_fixed, data$Y, nrow(data))
  mixed_rmse <- rmse(y_hat_mixed, data$Y, nrow(data))
  
  return(c(agreement, fixed_beta_rmse, mixed_beta_rmse, fixed_accuracy, mixed_accuracy,
...
rerun_single_params <- function(df) {
  ...
  mean_agreement <- mean(unlist(lapply(results, function (x) x[num_inputs + 1])))
  sd_agreement <- sd(unlist(lapply(results, function (x) x[num_inputs + 1])))
  
  mean_beta_rmse_fixed <- mean(unlist(lapply(results, function (x) x[num_inputs + 2])))
  sd_beta_rmse_fixed <- sd(unlist(lapply(results, function (x) x[num_inputs + 2])))
  
  mean_beta_rmse_mixed <- mean(unlist(lapply(results, function (x) x[num_inputs + 3])))
  sd_beta_rmse_mixed <- sd(unlist(lapply(results, function (x) x[num_inputs + 3])))
  
  mean_accuracy_fixed <- mean(unlist(lapply(results, function (x) x[num_inputs + 4])))
  sd_accuracy_fixed <- sd(unlist(lapply(results, function (x) x[num_inputs + 4])))
  
  mean_accuracy_mixed <- mean(unlist(lapply(results, function (x) x[num_inputs + 5])))
  sd_accuracy_mixed <- sd(unlist(lapply(results, function (x) x[num_inputs + 5])))
  
  mean_rmse_fixed <- mean(unlist(lapply(results, function (x) x[num_inputs + 6])))
  sd_rmse_fixed <- sd(unlist(lapply(results, function (x) x[num_inputs + 6])))
  
  mean_rmse_mixed <- mean(unlist(lapply(results, function (x) x[num_inputs + 7])))
  sd_rmse_mixed <- sd(unlist(lapply(results, function (x) x[num_inputs + 7])))
  ...
  names(row) <- c("reps", "j", "n", "type", "p", "s", "default", "cov_x", "mean_x", 
                  ...
                  "agreement", "fixed_beta_rmse", "mixed_beta_rmse", 
                  "fixed_accuracy", "mixed_accuracy", "fixed_rmse", "mixed_rmse")
