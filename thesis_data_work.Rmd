...
df_analysis_noncor <- outcomes_and_obs_full_y1_largec %>%
  filter(!is.na(outcomes_and_obs_full_y1_largec[[outcome]])) %>%
  dplyr::select(-c(all_of(other_outcomes), caretype, classid, cid, nsweeps.y,
                   actualtype, nsweeps.x)) 

df_analysis <- as.data.frame(df_analysis_noncor)

# cor_matrix <- rcorr(as.matrix(select_if(df_analysis[coefs_for_glm], is.numeric)))
# cor_df <- flattenCorrMatrix(cor_matrix$r, cor_matrix$P)
# cor_help <- cor_df[cor_df$row == coefs_for_glm[111], ]
```


```{r}
cor_matrix <- cor(as.matrix(select_if(df_analysis, is.numeric)))
