...
sigma_df <- data.frame("sigma_r" = seq(0, 4, by = 0.5))
# write.csv(dataset[[1]],
...
data <- read.csv(paste0(root, "min_lambda_sigma_r/1.csv"))
for (scale in scale_vec[-1]) {
  new <- read.csv(paste0(root, "min_lambda_sigma_r/", scale, ".csv"))
  data <- rbind(data, new)
}
...
file_saving_sim = function(chunkNo, seed, reps,
  ...
  {
    fname = paste0(root, "results/icc_new_outcome/",
               chunkNo, "_", seed, ".rds")
    res = NA
...
debugging = FALSE
sink(paste0(root, "logs/new_outcome.txt"))
...
pathname <- paste0(root, "results/icc_new_outcome")
if (TRUE) {
    # Run in parallel
