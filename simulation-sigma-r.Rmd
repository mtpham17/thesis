...
analyze <- function(train_data, test_data, p, true_beta, default, snr) {
  ...
  fixed_results <- perf_metrics(coefficients_fixed, true_beta, 
                                y_hat_fixed_train, train_data$Y,
                                y_hat_fixed_test, test_data$Y,
                                default, TRUE)
  mixed_results <- perf_metrics(coefficients_mixed, true_beta, 
...
one_run <- function(n_bar, J, alpha, 
  ...
  result <- analyze(train_data = generate$data, 
                    test_data = test$data,
                    p = generate$p, 
                    true_beta = generate$betas, 
...
file_saving_sim = function(chunkNo, seed, reps,
  ...
  {
    fname = paste0(root, "results/sigma_r_with_test/",
               chunkNo, "_", seed, ".rds")
    res = NA
...
debugging = FALSE
sink(paste0(root, "logs/sigma_r_with_test.txt"))
