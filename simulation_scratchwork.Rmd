...
rerun_single_params <- function(runs, m, j, beta_strength, min_beta, sigma_r, n=100) {
  results <- rerun(runs, one_run(m=10, j=2, beta_strength=0, 
                                 min_beta=0.1, sigma_r=0.2, n=20))
  ...
  row <- c(m, j, beta_strength, min_beta, sigma_r, n, runs, 
...
rerun_mult_parameters <- function(runs, m_vec, j_vec, beta_strength_vec, 
                                  min_beta_vec, sigma_r_vec, n_vec) {
  
  # test every possible combination of parameters, execute "runs" times
