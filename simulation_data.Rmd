...
```{r}
sigma_r_snr_plot <- ggplot(sigma_r_snr, mapping = aes(x = sigma_r^2,
                        y = mean_agreement,
                        group = snr_level,
                        color = snr_level)) +
  geom_line() + 
...
```{r}
metric_colnames <- colnames(sigma_r_snr %>%
                              dplyr::select(contains("y_rmse")))

subset <- sigma_r_snr %>%
  dplyr::select(c(metric_colnames), "snr_level", "sigma_r",
                    "mean_adj_r_squared", "mean_icc")
...
tidy_plots <- function(data, x, metric) {
  ...
  return(dataset_to_plot)
                 
  
}

tidy <- tidy_plots(sigma_r_snr, "sigma_r", "y_rmse")
